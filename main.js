...
const PALETTE_COLORS = [
  ...
  '#ff0000','#ff4040','#ff9900','#ffcc00','#00cc00','#00cccc','#0066ff','#6600cc',
  '#ff66cc','#cc0000','#cc6600','#cccc00','#006600','#006666','#003399','#330099',
  '#ffcccc','#ffe5cc','#ffffcc','#ccffcc','#ccffff','#cce5ff','#e5ccff','#ffcce5',
];
...
PALETTE_COLORS.forEach(c => {
  ...
  d.addEventListener('click', e => e.shiftKey ? setFillColor(c) : setStrokeColor(c));
  palEl.appendChild(d);
});
...
colorStrokeInput.addEventListener('input', e => setStrokeColor(e.target.value));
...
function bindSlider(id, valId, suffix, cb) {
  ...
  sl.addEventListener('input', () => { lbl.textContent = sl.value + (suffix||''); cb(+sl.value); });
}
...
document.getElementById('linecap-select').addEventListener('change', e => lineCap = e.target.value);
document.getElementById('dash-select').addEventListener('change', e => {
  dashPattern = e.target.value ? e.target.value.split(',').map(Number) : [];
});
document.getElementById('font-family-select').addEventListener('change', e => fontFamily = e.target.value);
...
function toggleStroke() {
  useStroke = !useStroke;
  document.getElementById('btn-stroke-on').classList.toggle('active', useStroke);
}
function toggleFill() {
  useFill = !useFill;
...
============================================================ */
const toolNames = {
  pencil:'Pensil', brush:'Kuas', eraser:'Penghapus', spray:'Semprot',
  fill:'Isi Warna', eyedropper:'Pipet Warna',
  line:'Garis', arrow:'Panah', rect:'Persegi', circle:'Lingkaran',
  triangle:'Segitiga', polygon:'Poligon', star:'Bintang', text:'Teks'
};
...
function setTool(tool) {
  ...
  document.getElementById('stat-hint').textContent = hints[tool] || 'Tekan Ctrl+Z untuk Undo';
  const cursorMap = { eraser:'cell', fill:'cell', eyedropper:'crosshair', text:'text' };
...
function resetCtx(c) {
  c.globalAlpha = 1; c.filter = 'none'; c.setLineDash([]);
...
function onMove(e) {
  ...
  if (currentTool === 'polygon' && polyPoints.length > 0) {
    drawPolyPreview(p.x, p.y); return;
  }
  if (currentTool === 'spray' && isDrawing) { startX = p.x; startY = p.y; return; }
  if (!isDrawing) return;
...
function drawShape(c, tool, x1, y1, x2, y2) {
  ...
  switch(tool) {
    ...
    case 'triangle':
      c.moveTo(x1+w/2, y1); c.lineTo(x2, y2); c.lineTo(x1, y2); c.closePath();
      if (useFill) c.fill();
      if (useStroke) c.stroke();
      break;
    case 'polygon':
      drawRegPoly(c, x1+w/2, y1+h/2, Math.min(Math.abs(w),Math.abs(h))/2, polygonSides);
      if (useFill) c.fill();
      if (useStroke) c.stroke();
      break;
    case 'star':
      drawStar(c, x1+w/2, y1+h/2, Math.min(Math.abs(w),Math.abs(h))/2,
               Math.min(Math.abs(w),Math.abs(h))/4, 5);
      if (useFill) c.fill();
      if (useStroke) c.stroke();
      break;
  }
...
function drawArrow(c, x1,y1,x2,y2) {
  ...
  const ha      = 0.42;
  c.moveTo(x1,y1); c.lineTo(x2,y2);
  c.lineTo(x2-headLen*Math.cos(angle-ha), y2-headLen*Math.sin(angle-ha));
  c.moveTo(x2,y2);
  c.lineTo(x2-headLen*Math.cos(angle+ha), y2-headLen*Math.sin(angle+ha));
  if (useStroke) c.stroke();
}
...
function roundRect(c,x,y,w,h,r) {
  const sx=w<0?x+w:x, sy=h<0?y+h:y, aw=Math.abs(w), ah=Math.abs(h);
  const rx=Math.min(r,aw/2), ry=Math.min(r,ah/2);
  c.beginPath();
  c.moveTo(sx+rx,sy); c.lineTo(sx+aw-rx,sy);
  ...
  c.lineTo(sx,sy+ry);
  c.quadraticCurveTo(sx,sy,sx+rx,sy); c.closePath();
}
...
function drawPolyPreview(mx, my) {
  ...
  applyStyle(octx);
  octx.beginPath();
  polyPoints.forEach((p,i) => i===0 ? octx.moveTo(p.x,p.y) : octx.lineTo(p.x,p.y));
  octx.lineTo(mx, my);
  if (useStroke) octx.stroke();
  // highlight first point
  octx.fillStyle = '#5c6ef8';
  octx.globalAlpha = 1; octx.filter='none';
  octx.beginPath(); octx.arc(polyPoints[0].x, polyPoints[0].y, 5, 0, Math.PI*2);
  octx.fill();
}
function finishPolygon() {
  if (polyPoints.length < 2) { polyPoints=[]; octx.clearRect(0,0,overlay.width,overlay.height); return; }
  octx.clearRect(0, 0, overlay.width, overlay.height);
  saveSnap();
  applyStyle(ctx);
  ctx.beginPath();
...
function sprayAt(x, y) {
  const r = brushSize*4+4, density = Math.max(10, brushSize*4);
  ...
  for (let i=0;i<density;i++) {
    ...
    ctx.beginPath();
    ctx.arc(x+d*Math.cos(a), y+d*Math.sin(a), Math.random()*1.5, 0, Math.PI*2);
    ctx.fill();
  }
  resetCtx(ctx);
}
...
function floodFill(px, py) {
  ...
  if (tr===fr && tg===fg && tb===fb && ta===fa) { showToast('Warna sudah sama'); return; }
  function match(i){return data[i]===tr&&data[i+1]===tg&&data[i+2]===tb&&data[i+3]===ta;}
  function paint(i){data[i]=fr;data[i+1]=fg;data[i+2]=fb;data[i+3]=fa;}
  const vis=new Uint8Array(W*H), stack=[[px,py]];
  while(stack.length){
...
function finishTextInput() {
  ...
  if (txt) {
    ...
    lines.forEach((line, i) => ctx.fillText(line, textPos.x, textPos.y + i*(fontSize*1.25)));
    resetCtx(ctx);
  }
  textEl.style.display='none'; textEl.value='';
}
textEl.addEventListener('keydown', e => {
  if (e.key==='Escape') { textEl.value=''; finishTextInput(); }
  if (e.key==='Enter' && !e.shiftKey) { e.preventDefault(); finishTextInput(); }
  setTimeout(() => { textEl.style.height='auto'; textEl.style.height=textEl.scrollHeight+'px'; },0);
});
...
function updateHistLabel() {
  document.getElementById('stat-history').textContent = `${histIdx}/${history.length-1}`;
}
...
function clearCanvas() {
  ...
  closeModal('modal-clear'); showToast('Kanvas dihapus ✓');
}
function resizeCanvas() {
  const nw = +document.getElementById('canvas-w').value;
  const nh = +document.getElementById('canvas-h').value;
  if (!nw||!nh||nw<10||nh<10) { showToast('Ukuran tidak valid'); return; }
  ...
  saveSnap(); showToast(`Kanvas diubah ke ${nw}×${nh}`);
}
...
function toggleGrid() {
  ...
  if (gridVisible) drawGrid(); else { gridCanvas&&gridCanvas.remove(); gridCanvas=null; }
}
function drawGrid() {
  if (!gridCanvas) {
    gridCanvas = document.createElement('canvas');
    gridCanvas.style.cssText = 'position:absolute;top:0;left:0;pointer-events:none;opacity:.2';
    wrapper.appendChild(gridCanvas);
  }
  gridCanvas.width=canvas.width; gridCanvas.height=canvas.height;
  const gc=gridCanvas.getContext('2d');
  gc.strokeStyle='#0088ff'; gc.lineWidth=0.5;
  const step=20;
  for(let x=0;x<=canvas.width;x+=step){gc.beginPath();gc.moveTo(x,0);gc.lineTo(x,canvas.height);gc.stroke();}
  for(let y=0;y<=canvas.height;y+=step){gc.beginPath();gc.moveTo(0,y);gc.lineTo(canvas.width,y);gc.stroke();}
}
...
function exportPNG() {
  ...
  a.download='gambar-canvasku.png'; a.href=canvas.toDataURL('image/png'); a.click();
...
function exportJPEG() {
  ...
  const a=document.createElement('a');
  a.download='gambar-canvasku.jpg'; a.href=tmp.toDataURL('image/jpeg',0.92); a.click();
...
document.addEventListener('keydown', e => {
  ...
  const map={
    'p':'pencil','b':'brush','e':'eraser','q':'spray',
    'g':'fill','i':'eyedropper',
    'l':'line','a':'arrow','r':'rect','c':'circle',
    't':'triangle','y':'polygon','*':'star','x':'text'
  };
  if (map[e.key.toLowerCase()]) setTool(map[e.key.toLowerCase()]);
  if (e.key==='Escape') { finishTextInput(); finishPolygon(); }
...
setTool('pencil');
setZoom(1);
document.getElementById('stat-size').textContent = `${canvas.width} × ${canvas.height}`;